...
<html lang="en">
  <head>
    ...
    <link href="css/boutton.css" rel="stylesheet" type="text/css" />
  ...
  <td>     <div class="ph-line-nav nav" class="menu">
   ...
   <td>  <ul class="headerButtons">
            <li class="respNav"><a href="#"><img src="img/icons/14x14/light/list.png" alt=""></a></li>
...
<script>
    ...
    var del=confirm("ete vous sur de supprimer ce compte?");
    if (del==true){
       }else{
        }
    return del;
...
<script>
    function updatetconfig(){

    var del=confirm("voulez vous le modifiez?");
    if (del==true){
}else{
...
if(!isset($_GET['id']))
	exit();
$Id=$_GET['id'];
$req = "select login,pass from compte where id_compte='".$Id."'";
$res = requete_ligne($req);
$err="";

if(isset($_POST['Supprimer'])){
	$req = "select id_compte from compte where id_compte='".$IdCom."'";
	$r = requete_ligne($req);
	$req = "delete from compte where id_compte='".$r[0]."'";
	$r1 = requete_execute($req);
    echo "<script>document.location = 'listecompte.php'</script>";
	exit();
}
?><center>
<h2 class="title">Suppression Compte</h2>
	<form action="" method="post">
	<input type="hidden" name="IdCom" value="<?php echo $Id?>">
	<table>
    <tr><td>Login: </td><td><input type="text" name="login" size="70" value="<?php echo $res['login']?>"></td></tr>
    <tr><td>Mot de passe : </td><td><input type="text" name="pass" size="70" value="<?php echo $res['pass']?>"></td></tr>
  <tr><td colspan=2>
    ...
    <input type="submit" name="Supprimer" value="Supprimer" onClick="return deleletconfig()">    
    <input type="submit" onClick="document.location='listecompte.php';" value="Retour"></button>
	</form></center>
