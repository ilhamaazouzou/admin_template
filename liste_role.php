...
<?php
$req = "select id_role, nom_role from role order by  nom_role";
$tab = requete_retour($req);
if(count($tab)==0)
	echo "aucun Role dans la liste";
else
{
?>
	
	<table class="liste" cellpadding=0 cellspacing=1 >
	<tr><th width="100"> Role</th><th colspan=2>Modification/Suppression</th></tr>
	<?php 
	...
	<?php if( haveDroit(2) && $tab[$i][0] != 1 && $tab[$i][0] != IdRole()) {?>
	<a href="modification_role.php?id=<?php echo $tab[$i][0]; ?>" title="Modifier"><img src="./image/b_edit.png" /></a>
	<?php
		}else{
	?>
	...
	<td align="center" style="width: 30px">
	<?php if( haveDroit(2) && $tab[$i][0] != 1 && $tab[$i][0] != IdRole()) {?>
	<a href="suppression_role.php?id=<?php echo $tab[$i][0]; ?>" title="Supprimer"><img src="./image/b_drop.png" /></a>
	<?php
		}else{
	?>
	...
	?>
    <tr><td colspan=2 align="center"><?php if( haveDroit(2)) {?>
	<input type="submit" name="submit" value="Ajouter" onClick="document.location='ajouter_role.php';"><?php }?></td><td><input type="submit" onClick="document.location='liste_role.php';"  value="Retour"></button></td><td></td></tr>
	</table>
