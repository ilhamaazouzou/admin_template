...
$err="";
if(isset($IdR) && isset($nom) && !empty($IdR) && !empty($nom)  ) {
		$req = "update role set nom_role = '".$nom."' where id_role='".$IdR."'";
		//echo $req;
		$r = requete_execute($req);
		
		$req = "delete from droit_role where id_role='".$IdR."'";
		$r = requete_execute($req);
...
?><center>
	<h2 class="title">Modifier Role</h2>
	...
	<tr><td valign="top">Droits:</td><td>	
		<table>
			<?php	
				...
				$req = "select id_droit from droit_role where id_role = ".$Id;
				$tabD = requete_colonne($req);
				$req = "select id_droit, nom_droit from droit where droitglobal =''";
				$tab = requete_retour($req);
				for($i=0;$i<count($tab);$i++){
					$checked = (in_array($tab[$i][0],$tabD))? "checked='checked'" : "";
					echo "<tr><td>
						<div class='squaredOne'><input type='checkbox' value='".$tab[$i][0]."' id='squaredOne".$tab[$i][0]."' name='droits[]' ".$checked."/>
		...
		//<input type=\"checkbox\" name=\"droits[]\" value=\"".$tab[$i][0]."\" />
					...
					$checked = (in_array($tab2[$j][0],$tabD))? "checked='checked'" : "";
					echo "<tr><td></td><td>
						<div class='squaredOne'><input type='checkbox' value='".$tab2[$j][0]."' id='squaredOne".$tab2[$j][0]."' name='droits[]' ".$checked." />
						<label for='squaredOne".$tab2[$j][0]."'></label></div>
						</td><td><label class='lSquaredOne' for='squaredOne".$tab2[$j][0]."'>".$tab2[$j][1]."</label></td></tr>";
						
					//echo "<br/><input type=\"checkbox\" name=\"droits[]\" value=\"".$tab[$i][0]."\" ".$checked." />".$tab[$i][1];
					}
				}
			?>
		</table>
	</td></tr>
	<tr><td colspan=2 align="center"><input type="submit" value="Modifier"><input type="submit" onClick="document.location='liste_role.php';"value="retour"></button></td></tr>
	<?php if(strlen($err)) {?>
